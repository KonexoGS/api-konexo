...
def get_pib_from_country(info: Literal["PIB", "Populacao", "Desigualdade(GINI)", "Trabalhadores Agro", "ForcaTotal Trabalho"], country_iso_code: str, year: Annotated[str | None, Query(max_length=5)] = None):
    try:
        if len(country_iso_code) > 3 or len(country_iso_code) <= 1:
            raise HTTPException(status_code=400, detail="O código ISO do país deve conter de duas a três caractéres.")
